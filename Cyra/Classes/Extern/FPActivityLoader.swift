...
//
//  modificated by Darek Dominiak on 21/08/2016
...
class FPActivityLoader: UIView {
    ...
    static let kWidthActivityLoader: CGFloat = 40
    static let kHeightActivityLoader: CGFloat = 40
    
    static let defaultColor = UIColor.cyraPink()
    static let defaultLineWidth: CGFloat = 5.0
    static let defaultCircleTime: Double = 1.8
    
    class func addActivityLoader(view: UIView) -> UIView
    {
        let centerX = view.bounds.width / 2 - (kWidthActivityLoader / 2)
        let centerY = view.bounds.height / 2 - (kHeightActivityLoader / 2)
        let centerPoint: CGPoint = CGPoint(x: centerX, y: centerY)
        let activityLoader = FPActivityLoader(frame: CGRect(origin: centerPoint, size: CGSize(width: kWidthActivityLoader, height: kHeightActivityLoader)))
        view.addSubview(activityLoader)
        return activityLoader
    }
    
    class func hideActivityLoaderForView(view: UIView)
    {
        let subviews = view.subviews
        for v in subviews
        {
            if v .isKindOfClass(FPActivityLoader)
            {
